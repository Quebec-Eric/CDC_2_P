...
public  class PilhaAlocaFlex {
      ...
      for (int i = 0; i < variarAloca; i++) {
        for (int z = 0; z < y; z++) {
          if (comTudo[i].equals(classMusicas[z].getId())) {
            		pilha.inserir(classMusicas[z]);
          }
        }
      }
     pilha=  fazerImpilhacao(pilha,classMusicas,y,tamahho,tyr,variarAloca ,mudancasLista);
 ...
 public static Pilha fazerImpilhacao( Pilha impilharMusicas,Musica []classMusicas,int y,int tamahho, int tyr,
  int variarAloca ,String []mudancasLista)  throws IOException{
        ...
        for (int t = variarAloca; t < tyr; t++) {
            for (int z = 0; z < y; z++) {
              if (eric[1].contains(classMusicas[z].getId())) {
                impilharMusicas.inserir(classMusicas[z]);
                t = tyr + 1;
  							z = y + 1;
              }
            }
    ...
    if (eric[0].equals("R")) {
			Musica pegar =  impilharMusicas.remover();
