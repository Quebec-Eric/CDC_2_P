...
*/
 ...
 class Pilha {
  ...
  private int n;
  private int inicio;
  public Pilha()throws IOException {
    this(3);
  }
  /*  Inico da Class Lista passando com o paramentro */
  public Pilha(int tamanho)throws IOException  {
    quebecArray = new Musica[tamanho];
    n = 0;
  }

  public Musica removerPilha()throws IOException{
    if (n==0){
	     throw new IOException("Erro ao Remover do Inicio!");
    }
    else{
  ...
  public void colocarNaPilha(Musica colocar)throws IOException{

    //  System.out.println("-- " + colocar.toString());
    if (n>quebecArray.length){
       throw new IOException("Erro ao Remover do Inicio!");
    }
    else{
        quebecArray[n]=colocar;
       n++;
    }
  }
...
public class PilhaAloca {
	...
	public static void main(String[] args) throws IOException {
      ...
      /* Abrir arquivo csv para leiturea */
		try { /// tmp/data.csv
      ...
      Pilha impilharMusicas = new Pilha(variarAloca + saberTudo);
      for (int i = 0; i < variarAloca; i++) {
        for (int z = 0; z < y; z++) {
          if (comTudo[i].equals(classMusicas[z].getId())) {
            impilharMusicas.colocarNaPilha(classMusicas[z]);
          }
        }
      }
      impilharMusicas=  fazerImpilhacao(impilharMusicas,classMusicas,y,tamahho,tyr,variarAloca ,mudancasLista);

      impilharMusicas.mostrarPilha();
		} catch (IOException e) {
 ...
 public static Pilha fazerImpilhacao( Pilha impilharMusicas,Musica []classMusicas,int y,int tamahho, int tyr,
  int variarAloca ,String []mudancasLista)  throws IOException{
    String inserir = "I";
  	String remover = "R";
    for (int i = 0; i < tamahho; i++) {
      String[] eric = mudancasLista[i].split(" ");
    ...
    if (eric[0].equals("R")) {
			Musica pegar =  impilharMusicas.removerPilha();
