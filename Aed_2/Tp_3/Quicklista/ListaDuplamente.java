...
//***************************************************************************
 /*  Inicio class Fazer elementos*/
	class CelularDupla{
		...
		public CelularDupla anterior;
		public CelularDupla proximo;

		public CelularDupla(){
			this(null);
		}
		public CelularDupla(Musica musica){
...
*/
	class ListaDupla{
		...
		private CelularDupla primeiro;
		private CelularDupla ultimo;

	public 	ListaDupla(){
			primeiro = new CelularDupla();
			ultimo=primeiro;
		}

		public void inserirInicio(Musica musica) {
				CelularDupla temporario = new CelularDupla(musica);
		      temporario.anterior = primeiro;
		      temporario.proximo = primeiro.proximo;
		      primeiro.proximo = temporario;
		      if(primeiro == ultimo){
		         ultimo = temporario;
		      }else{
			...
			public void inserirFim(Musica  musica) {
				ultimo.proximo = new CelularDupla(musica);
      	ultimo.proximo.anterior = ultimo;
				ultimo = ultimo.proximo;
			}

			public void mostrar() {
						for (CelularDupla i = primeiro.proximo; i != null; i = i.proximo) {
							System.out.println(i.musica.toString());
						}
			}

			public int tamanho(){
				int tamanho=0;
				for (CelularDupla i = primeiro.proximo; i != null; tamanho++,i = i.proximo);
				return tamanho;
			}
			...
			public CelularDupla getelementoPos(int posicao)
     {
         CelularDupla i=primeiro.proximo;
      ...
      //  for(int k = 0; k < (numero1+numeor2+numero3)/3 && pivo.proximo != null; pivo = pivo.proximo);
			//	System.out.println(pivo);
			 int pivo=getelementoPos((dir+esq)/2).musica.getDuration_ms();
				  while (i <= j) {
							while(getelementoPos(i).musica.getDuration_ms() <pivo)i++;
							while(getelementoPos(j).musica.getDuration_ms()>pivo)j--;
			 ...
			 public void swap(CelularDupla i, CelularDupla j)
		     {
		         Musica tmp=i.musica;
		         i.musica=j.musica;
		         j.musica=tmp;
...
public class ListaDuplamente {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			ListaDupla eric = new ListaDupla();
			for (int p = 0; p < variarAloca; p += 1) {
				for (int j = 0; j < variarAloca; j += 1) {
					if (resposta1[j].getId().contains(entradaDados[p])) {
						eric.inserirFim(resposta1[j]);
					}
				}
			}
			//System.out.println(eric.tamanho());
