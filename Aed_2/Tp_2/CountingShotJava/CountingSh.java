...
public class CountingSh {
	...
	public static void ajustarParaOCoun(Musica[] resposta) {
		...
		resposta = coutingShotg(resposta);
	...
	public static String voltarAoNormal(String linhas) {
		int saber = linhas.indexOf("|");
		String retirada = linhas.substring(0, saber);
		return retirada;

	}

	/* Ordenacao */
	public static Musica[] coutingShotg(Musica[] resposta) {
		int n = resposta.length;
		int[] count = new int[getMaior(resposta) + 1];
		Musica[] ordenado = new Musica[resposta.length];
		...
		for (int i = 0; i < n; i++)
			// count[array[i]]++
			count[resposta[i].getPopularity()]++;

		for (int i = 1; i < count.length; i++)
			count[i] += count[i - 1];

		for (int i = n - 1; i >= 0; i--) {
			ordenado[count[resposta[i].getPopularity()] - 1] = resposta[i];
			count[resposta[i].getPopularity()]--;
		}

		for (int i = 0; i < n; i++)
			resposta[i] = ordenado[i];
		return resposta;
	}

	public static String pegarPopulary(String linha) {
		int saber = linha.indexOf("|");
		String retirada = linha.substring(saber + 2, linha.length());
		return retirada;
	}

	/* pegar o maior valor */
	public static int getMaior(Musica[] resposta) {
		int maior = resposta[0].getPopularity();

		for (int i = 0; i < resposta.length; i++) {
			if (maior < resposta[i].getPopularity()) {
				maior = resposta[i].getPopularity();
			}
		}
		return maior;
	}
