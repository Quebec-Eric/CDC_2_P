...
public class MergesortJ {
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			/* ajustar para fazer o MergesortJ */
	...
	public static void ajustarParaOCoun(Musica[] resposta) {
		...
		resposta = mergesort(resposta, 0, resposta.length - 1);
	...
	/* Ordenar */
	private static Musica[] mergesort(Musica[] resposta, int esq, int dir) {
		if (esq < dir) {
			int meio = (esq + dir) / 2;
			mergesort(resposta, esq, meio);
			mergesort(resposta, meio + 1, dir);
			resposta = intercalar(resposta, esq, meio, dir);
		}
		return resposta;
	}

	public static Musica[] intercalar(Musica[] resposta, int esq, int meio, int dir) {

		// Definir tamanho dos dois subarrays
		int n1 = meio - esq + 1;
		int n2 = dir - meio;

		Musica[] esquerda = new Musica[n1 + 1];
		Musica[] direita = new Musica[n2 + 1];

		// Inicializar primeiro subarray
		for (int i = 0; i < n1; i++) {
			esquerda[i] = resposta[esq + i];
		}

		// Inicializar segundo subarray
		for (int j = 0; j < n2; j++) {
			direita[j] = resposta[meio + j + 1];
		}
		int i = 0;
		int j = 0;
		int k = esq;
		while (i < n1 && j < n2) {
			if (esquerda[i].getEnergy() <= direita[j].getEnergy())

			{
				resposta[k] = esquerda[i];
				i++;
			} else {
				resposta[k] = direita[j];
				j++;
			}
			k++;
		}
