...
public class Clajava {
		...
		try {
			...
			for (int p = 0; p < variarAloca; p += 1) {
				for (int j = 0; j < variarAloca; j += 1) {
					if (saidas[j].contains(entradaDados[p])) {
	...
	public static Musica extrairMusica(String linha) {
		...
		if (linha.contains("\"")) {
			...
			if (respostafinal[12] != null) {
				musica.setRealease_date(respostafinal[12]);
			}
			...
			if (resposta[12] != null) {
				musica.setRealease_date(resposta[12]);
			}
		...
		if (tirarVirgula.length > 1) {
			for (int i = 0; i < tirarVirgula.length; i++) {
				if (tirarVirgula[i].charAt(1) == tt && tirarVirgula[i].charAt(2) == tt) {
					...
					nomes.add(tirarVirgula[i].substring(3, tirarVirgula[i].length() - 2));
