...
public class QuicksortParc {
			...
			while ((linhaArquivo = conteudoCsv.readLine()) != null) {
				for (int z = 0; z < variarAloca; z++) {
					...
					if (linhaArquivo.contains(entradaDados[z])) { /* pegar so as linhas que foram iguais as entradas */
						resposta1[z] = extrairMusica(
								linhaArquivo); /* receber na class as respostas de todas fazendo um array de Class */
	...
	public static Musica[] quicksort(Musica[] resposta, int esq, int dir) {
		...
		if (dir - esq > 0) {
			...
			int pivo = resposta[(dir + esq) / 2].getDuration_ms();
			while (i <= j) {
				while (resposta[i].getDuration_ms() < pivo)
					i++;
				while (resposta[j].getDuration_ms() > pivo)
					j--;
				if (i <= j) {
					swap(resposta, i, j);
					i++;
			...
			if (esq < j)
				quicksort(resposta, esq, j);
			if (i < dir)
				quicksort(resposta, i, dir);
		}
		return resposta;
	}
