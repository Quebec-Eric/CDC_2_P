...
public static void heapFazer(String[] saidas) {
        ...
        for (int i = 0; i < saidas.length; i++) {
                pato[i] = pegarData(saidas[i]);

        }
        pegarLocalDate = new String[pato.length];
        ...
        pegarLocalDate= fazerHEap();
        // System.out.println(Arrays.toString(pegarLocalDate));

        for (int i = 0; i < pato.length; i++) {
                pegarLocalDate[i] = mudarS(pegarLocalDate[i].replace("-", "/"));
                // System.out.println(pegarLocalDate[i]);
        }

        for (int p = 0; p < pegarLocalDate.length; p += 1) {
                for (int j = 0; j < saidas.length; j += 1) {
                        if(saidas[j].contains(pegarLocalDate[p])) {
...
public static String pegarData(String linha) {
        String nomes = "";
        int inicioaSerCortado = linha.indexOf("/");
        if (linha.charAt(inicioaSerCortado + 3) == '/') {
                int fimaSerCortado = linha.indexOf("#", inicioaSerCortado);
                nomes = linha.substring(inicioaSerCortado - 2, fimaSerCortado - 1);
                // System.out.println("ola");
                return nomes;
        } else {
                inicioaSerCortado = linha.indexOf("/", inicioaSerCortado + 1);
                int fimaSerCortado = linha.indexOf("#", inicioaSerCortado);
                nomes = linha.substring(inicioaSerCortado - 2, fimaSerCortado - 1);
        }
        return nomes;
...
public static String [] fazerHEap() {
        String [] temp= new String[pegarLocalDate.length+1];

        for(int i = 0; i < pegarLocalDate.length; i+=1) {
                temp[i+1]=pegarLocalDate[i];
        }
        pegarLocalDate=temp;
        //System.out.println(Arrays.toString(pegarLocalDate));
        //temp[0]= null;
        //System.out.println(Arrays.toString(temp)); // olhar depoiis doc array ;

        for (int tamHeap = 2; tamHeap <=pegarLocalDate.length-1; tamHeap++) {
                construir(tamHeap, temp);
        }
        //System.out.println("================");
        //System.out.println(Arrays.toString(temp));
        int tamHeap = temp.length-1;
        while (tamHeap > 1) {

                swap(temp, 1, tamHeap--);
                reconstruir(tamHeap, temp);
                //System.out.println(Arrays.toString(temp));
        }
        //System.out.println("-------- ");

        temp=pegarLocalDate;
        String [] sapo= new String [temp.length-1];
        for(int i =0; i<temp.length-1; i+=1) {
                sapo[i]=temp[i+1];
        }
        //System.out.println(Arrays.toString(sapo));

        return sapo;
}

public static void construir(int tamanho, String [] temp){
        for(int i = tamanho; i > 1 && temp[i].compareTo(temp[i / 2]) > 0; i/=2) {
                swap(temp,i, i/2);
        }
}

public static void swap(String [] temp,int i, int j) {
        String t = temp[i];
        temp[i] = temp[j];
        temp[j] = t;
}
public static void reconstruir(int tamHeap, String [] temp){
        int i = 1;
        while(i <= (tamHeap/2)) {
                int filho = getMaiorFilho(i, tamHeap, temp);
                if(temp[i].compareTo(temp[filho])<0) {
                        swap(temp,i, filho);
                        i = filho;
                }else{
                        i = tamHeap;
...
public static int getMaiorFilho(int i, int tamHeap, String [] temp){
        int filho;
        if (2 * i == tamHeap || (temp[2 * i].compareTo(temp[2 * i + 1])) > 0) {
                filho = 2*i;
        } else {
                filho = 2*i + 1;
        }
...
public static String refazerData(String pato) {
        String[] eric = pato.split("/");
        String fazer = "";
        for (int i = eric.length - 1; i >= 0; i--) {
                fazer += eric[i];
                if (i > 0) {
                        fazer += "-";
                }
        }
        //LocalDate aLD = LocalDate.parse(fazer);
        return fazer;
}
